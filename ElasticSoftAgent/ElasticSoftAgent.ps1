...
foreach ($soft in $software){
    ...
    $Object | Add-Member -MemberType NoteProperty -Name $soft.DisplayName.replace(' ', '_').replace('"', '').replace("-","_").replace("(", "").replace(")", "").ToLower() -Value $soft.DisplayVersion
            
}

$list = ConvertTo-JSON $Object
...
Invoke-RestMethod -Uri $uri -Method Post -Body $list -Headers $header